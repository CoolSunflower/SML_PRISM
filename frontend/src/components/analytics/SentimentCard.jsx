...
function SentimentBar({ label, count, total, color, bgColor }) {
...
export function SentimentCard({ data }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Sentiment</p>
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 text-center">
        <span className="text-xs text-slate-400">{formatNumber(total)} total analyzed</span>
      </div>
    </div>
  );
}
